...
if (empty($_SESSION)) {
                    ...
                    <div class="offcanvas-body">
                        <ul class="navbar-nav justify-content-center flex-grow-1 pe-3">
                            <?php if ($_SESSION['fonction'] == 'eleve') { ?>
                                <li class="nav-item">
                                    <a class="nav-link mx-lg-2 active" aria-current="page" href="newAnuaireProf.php">Annuaire</a>
                                </li>
    ...
    <main class="mt-5 pt-5">
        <section class="card shadow p-4">
            <header class="mb-4">
                <h2 class="text-center text-primary">Liste des Professeurs</h2>
            </header>
            <div class="table-responsive">
                <table id="myTable" class="table align-middle mb-0 bg-white">
                    ...
                    $requete = $bdd->prepare("
    SELECT u.nom AS u_nom, u.prenom AS u_prenom, u.email AS u_email, u.fonction AS u_fonction, 
           h.nom AS h_nom, s.nom AS s_nom 
    FROM utilisateur AS u
    INNER JOIN hopital AS h ON u.ref_hopital = h.id
    INNER JOIN specialite AS s ON u.ref_specialite = s.id
    WHERE u.inscrit = 1;
");
                    $requete->execute();
                        ...
                        <tr>
                            <td>
                                <div class="d-flex align-items-center">
                                    ...
                                    />
                                    <div class="ms-3">
                                        <p class="fw-bold mb-1"><?php echo htmlspecialchars($ligne['u_nom']) . ' ' . htmlspecialchars($ligne['u_prenom']); ?></p>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <p class="fw-normal mb-1"><?php echo htmlspecialchars($ligne['u_fonction']); ?></p>
                            </td>
                            <td>
                                <span class="fw-normal mb-1"><?php echo htmlspecialchars($ligne['s_nom']); ?></span>
                            </td>
                            <td>
                                <button type="button" class="btn btn-outline-primary btn-sm btn-rounded" disabled>
                                    <?php echo htmlspecialchars($ligne['u_email']); ?>
                                </button>
                            </td>
                            <td>
                                <span class="fw-normal mb-1"><?php echo htmlspecialchars($ligne['h_nom']); ?></span>
                            </td>
                        </tr>
                    <?php } ?>
                    </tbody>
